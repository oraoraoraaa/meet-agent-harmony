...
\newcommand{\TeamName}{MeetAgent 团队}
...
\begin{itemize}
  ...
  \item \textbf{体验：}深色液态玻璃、搜索优先选点、结果页切换方案即更新路线、数据源徽章诚实可见。
\end{itemize}
...
\begin{tikzpicture}[
  ...
  layer/.style={draw=maDark!60, rounded corners=4pt, align=center,
    inner sep=6pt, fill=white, minimum height=1.0cm},
  arr/.style={-{Latex}, thick, maGray}
]
  \node[layer, minimum width=12.2cm, fill=maSoft] (ui)
    {\textbf{ArkUI 界面层}\\主页地图 · 普通规划 · 智能助手 · 锁定会话};
  \node[layer, minimum width=12.2cm, below=0.4cm of ui, fill=blue!6]
    (agent) {\textbf{Agent 编排}\\工具循环 · 接地校验 · Mode A/B/C};
  \node[layer, minimum width=5.7cm, below left=0.5cm and -0.1cm of agent.south, fill=green!6]
    (tools) {\textbf{工具主机}\\驾车/步行/骑行/公交\\候选 · 评分 · regeo};
  \node[layer, minimum width=5.7cm, below right=0.5cm and -0.1cm of agent.south, fill=orange!8]
    (engine) {\textbf{本地引擎 domain/}\\路径拦截 · 多模式评分};
  \node[layer, minimum width=12.2cm, below=1.45cm of agent, fill=maSoft]
    (map) {\textbf{Hybrid MapProvider}\\实时 AMap REST/JS · 估算回退 · 诚实 dataSource};

  \draw[arr] (ui) -- (agent);
  \draw[arr] (agent.south) -- ++(0,-0.12) -| (tools.north);
  \draw[arr] (agent.south) -- ++(0,-0.12) -| (engine.north);
  \draw[arr] (tools.south) -- ++(0,-0.22) -| (map.north);
  \draw[arr] (engine.south) -- ++(0,-0.22) -| (map.north);
\end{tikzpicture}
...
\textbf{硬规则：}界面分钟数、折线、坐标只能源自工具/引擎。
...
\begin{itemize}
  ...
  \item \textbf{估算：}无 Key 或 Fixture 开启——弱网/赛场保底。
  \item \textbf{实时+估算：}部分模式失败回退，徽章如实标注。
\end{itemize}
...
\vspace{0.4em}
\noindent{\zihao{5}\color{maGray}
（介绍文档正文约 720 字，符合不超过 800 字；以下为补充材料。）
}
...
\begin{enumerate}
  ...
  \item 备选：智能助手自然语言约束（如“最多走 8 分钟，不要公交”）。
\end{enumerate}
...
\begin{lstlisting}[language=bash,caption={领域层测试}]
cd domain && npm test
\end{lstlisting}

\begin{itemize}
  \item DevEco Studio 打开仓库根目录，运行 \texttt{entry}。
  \item 设置中配置 Map Web Key / LLM；详见 \texttt{README.md}。
\end{itemize}
...
\begin{enumerate}
  ...
  \item 本仓库 \texttt{docs/PRODUCT.md}、\texttt{docs/ARCHITECTURE.md}、\texttt{docs/AI\_AGENT.md}。
\end{enumerate}

\vspace{1.2em}
\begin{center}
{\zihao{5}\color{maGray}
—— 正文完 ——
}
\end{center}
